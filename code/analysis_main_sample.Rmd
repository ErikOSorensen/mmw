...
ggsave("graphs/MS31587_Figure-1.pdf",width=16, height = 10, units = "cm", device=cairo_pdf)
```
...
ggsave("graphs/MS31587_Figure-2.pdf", width=16, height = 10, units="cm", device=cairo_pdf)
```
...
ggsave("graphs/MS31587_Figure-3.pdf", width=16, height = 12, units="cm", device=cairo_pdf)
```
...
# Minimal winning margin across the distribution of winner performance

This analysis is restricted to the 400 observations in which $x_2 - x_1==1$. 
A regression table approach:

```{r winning_margin_over_distribution, echo=FALSE}
tar_load(winning_margin_over_distribution_l)

tX <- winning_margin_over_distribution_l$regs |>
  modelsummary(coef_map=winning_margin_over_distribution_l$names,
               stars = c('*' = .1, '**' = .05, '***'=.01),
                                   gof_map = c("nobs", "r.squared"),
                                   output = "gt") |>
  tab_spanner(label="All to Winner", columns=2:4) |>
  tab_spanner(label="Share to Winner", columns=5:7)
tX
tX |> gtsave(here::here("tables","RN2018_winning_margin_over_distribution.tex"))
```
